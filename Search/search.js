...
var search = new Vue({
	...
	data: {
		...
		// Flag to indicate of a search has been conducted
		isSearchConducted: true,
	...
	computed: {
		filteredList() {
			...
			if (this.searchQueryInput == '') {
				// Assign the "searchQueryResults" array to an empty array
				this.searchQueryResults = '';
			}

			// If the "searchResults" array is empty
			if (this.searchQueryResults == '') {
				// Switch the "isSearchConducted" flag to true so that the 'Pre-search message' is displayed
				this.isSearchConducted = true;
			}
			else {
